...
- **インストールは安全機構つき**——インストール前の依存監査、インストール後の起動構成チェック、失敗時は自動ロールバック。壊れたプラグインで DSH が起動不能になることはありません（詳細は `README.zh.md` の「安全机制（兜底）」を参照）。
...
4. **カード操作**：
   ...
   - インストール済みプラグインには「更新」の右に **アンインストール** ボタンもあります。
