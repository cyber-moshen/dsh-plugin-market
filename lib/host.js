...
let filling = null
/** Background auto-update loop state. */
let autoState = null
...
/** Sequential auto-update loop over { depKey, profile, label } entries. */
async function runAutoUpdates(queue) {
  autoState = { active: true, done: 0, total: queue.length, current: null }
  for (const item of queue) {
    while (activeOp && activeOp.status === 'running') await sleep(2000)
    if (autoState === null) return
    autoState.current = item.label
    const binInfo = resolveDshBin()
    if (binInfo) {
      const res = await runChild(binInfo, ['plugin', '--profile', item.profile, 'update', item.depKey, ...workspaceFlag()], profileDir(item.profile), DEFAULT_TIMEOUT)
      if (res.code === 0) advanceBaseline(item.profile, item.depKey)
    }
    autoState.done++
  }
  autoState = null
...
function autoUpdateSnapshot() {
  return autoState ? { active: autoState.active, done: autoState.done, total: autoState.total, current: autoState.current } : null
}
    ...
    handler: async (req, res) => {
      try {
        ...
        if (method === 'list') {
          ...
          if (cfg.autoUpdate && !autoState) {
            const pending = base.plugins
              .filter((p) => p.inst && p.inst.state === 'update' && p.inst.depKey)
              .map((p) => ({ depKey: p.inst.depKey, profile: p.inst.profile, label: p.name }))
            if (pending.length > 0) runAutoUpdates(pending)
          }
          base.config = { tokenSet: !!githubToken(), autoUpdate: !!cfg.autoUpdate }
          base.autoUpdate = autoUpdateSnapshot()
