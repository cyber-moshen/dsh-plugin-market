...
async function fetchRepoStats(owner, repo) {
  ...
  const headers = { 'User-Agent': 'dsh-plugin-market/0.1.2', 'Accept': 'application/vnd.github+json' }
...
async function runBootUpdater() {
  try {
    ...
    const applyUpdate = async (profile, depKey, label) => {
      ...
      const res = await runProcess(binInfo, ['plugin', '--profile', profile, 'update', depKey, ...workspaceArgs()], profilePath(profile), TASK_TIMEOUT)
      if (res.code === 0) {
