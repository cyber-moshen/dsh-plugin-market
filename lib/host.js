...
async function runFill(repos) {
  ...
  for (const { owner, repo } of repos) {
    ...
    const stale = !existing || Date.now() - (existing.fetchedAt || 0) >= CACHE_TTL
    // Re-fetch shields-only entries missing a push date when the API is up
    // (shields' last-commit badge is unreliable; stars may be present but
    // pushedAt null — the GitHub API fills it properly).
