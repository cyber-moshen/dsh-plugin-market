...
let liveTask = null
let taskCounter = 0
/** In-memory catalog mirror (TTL-cached). */
let catalogMirror = null
/** Stats/version cache: { repos, versions, baselines, ghLimitedUntil }. */
let store = null
...
function storePath() {
  return join(homeDir(), '.dsh-plugin-market-cache.json')
}

function legacyStorePath() {
  return join(homeDir(), '.dsh-market-card-cache.json')
}
...
/** The bundled catalog snapshot (this package's data/plugins.json). */
...
/**
 * Load the catalog: the repo's data/plugins.json over the mirror chain, else
 * the bundled snapshot. Returns { data, source } ('repo' | 'snapshot').
 */
async function loadCatalog() {
  ...
  try {
    const data = JSON.parse(readFileSync(bundledCatalogPath(), 'utf8'))
    catalogMirror = { at: Date.now(), data, source: 'snapshot' }
    return catalogMirror
  } catch (e) {
...
function loadStore() {
  ...
  let raw = null
  try { raw = readFileSync(storePath(), 'utf8') } catch {}
  if (raw === null) { try { raw = readFileSync(legacyStorePath(), 'utf8') } catch {} }
  if (raw !== null) {
    try { store = JSON.parse(raw) } catch {}
  }
  if (!store || typeof store.repos !== 'object') store = { repos: {}, ghLimitedUntil: null }
  if (typeof store.versions !== 'object') store.versions = {}
  if (typeof store.baselines !== 'object') store.baselines = {}
  return store
}

function persistStore() {
  try { writeFileSync(storePath(), JSON.stringify(loadStore())) } catch {}
}
...
async function fetchRepoMeta(owner, repo) {
  const s = loadStore()
  const ghLimited = !!s.ghLimitedUntil && Date.now() < s.ghLimitedUntil
  const hasToken = !!githubToken()
  if (!ghLimited || hasToken) {
    const res = await fetchRepoStats(owner, repo)
    if (res.limited) {
      s.ghLimitedUntil = res.resetAt
      persistStore()
    }
...
/** Background stats backfill: walk the catalog, top up stale/missing entries. */
async function backfillStats(repos) {
  ...
  for (const { owner, repo } of repos) {
    ...
    const existing = s.repos[key]
    const stale = !existing || Date.now() - (existing.fetchedAt || 0) >= STAT_TTL
    ...
    const shieldsMissingPush = !!(existing && existing.src === 'shields' && !existing.pushedAt)
    if (!stale && !shieldsMissingPush) continue
    const res = await fetchRepoMeta(owner, repo)
    if (res && res.ok) s.repos[key] = res.meta
    else if (res && res.missing) s.repos[key] = { missing: true, fetchedAt: Date.now() }
    done++
    if (done % 8 === 0) persistStore()
    await delay(250)
  }
  persistStore()
}
...
function kickoffBackfill(catalog) {
  ...
  const pending = repos.filter(({ owner, repo }) => {
    ...
    return !e || Date.now() - (e.fetchedAt || 0) >= STAT_TTL || (e.src === 'shields' && !e.pushedAt)
  })
  // Prune leftover stats from older catalog versions (keeps the cache file tidy).
  const known = new Set(repos.map(({ owner, repo }) => `${owner}/${repo}`))
  let pruned = false
  for (const key of Object.keys(s.repos)) {
    if (!known.has(key)) { delete s.repos[key]; pruned = true }
  }
  if (pruned) persistStore()
  if (pending.length === 0) return
  backfillInFlight = { active: true }
  const done = (v) => { backfillInFlight = null; return v }
  backfillStats(pending).then(done, done)
}

/** Merge catalog + cached stats into the client payload. */
function assembleCatalog(catalog, lang, catalogSource) {
  ...
  return {
    ...
    enriched: {
      ...
      running: !!(backfillInFlight && backfillInFlight.active),
    },
    env: environmentInfo(),
  }
}
...
async function queryLatestVersion(plugin, s) {
  ...
  s.versions[key] = { version, checkedAt: Date.now() }
  persistStore()
  return version
}
...
function advanceBaseline(profile, depKey) {
  try {
    ...
    const s = loadStore()
    const latest = s.versions && s.versions[fp] && s.versions[fp].version
    if (latest) {
      if (!s.baselines) s.baselines = {}
      s.baselines[fp] = { version: latest, at: Date.now() }
      persistStore()
    }
  } catch {}
}
...
function environmentInfo() {
  ...
  return {
    ...
    ghLimited: !!(s.ghLimitedUntil && Date.now() < s.ghLimitedUntil),
  }
}
