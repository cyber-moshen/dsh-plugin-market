...
// ------------------------------------------------------- boot-safety net
//
// The catastrophic failure mode of this market is a plugin that leaves the
// profile in a state where the NEXT boot dies hard ("plugin tree failed to
// load: duplicate loader entry id …"). The official `dsh plugin` reconcile
// turns every dependency that declares `dsh.bundle` into its own profile
// layer, so an aggregate package whose dependencies are also bundles (e.g.
// the dsh-web-ui family) routinely inserts the same loader row twice and
// bricks boot. Two layers of defense:
//
//   1. a budgeted pre-install audit that flags "bundle fan-out" (dependencies
//      that will each become a profile layer) and requires an explicit
//      confirmation;
//   2. the authoritative gate: after every install/update, the persisted
//      bundle stack is composed in a throwaway subprocess (the same
//      composition the boot performs — scripts/compose-check.mjs). If it
//      fails, the operation is automatically rolled back to the pre-op
//      manifest, so a broken plugin can never leave the profile unbootable.

const COMPOSE_CHECK_SCRIPT = fileURLToPath(new URL('../scripts/compose-check.mjs', import.meta.url))
const COMPOSE_CHECK_TIMEOUT = 20000
const REPAIR_HINT = '若 profile 已无法启动且需要离线修复，可在任意终端运行：node <dsh-plugin-market 目录>/scripts/repair.mjs --profile '

/** Pre-op snapshot of the profile manifest (the rollback source of truth). */
function snapshotProfile(profile) {
  try {
    const f = profilePath(profile) + '/package.json'
    return { path: f, content: readFileSync(f, 'utf8'), at: Date.now() }
  } catch { return null }
}

/** Restore a snapshot taken by snapshotProfile. Returns whether it succeeded. */
function restoreProfile(snap) {
  if (!snap || !snap.path) return false
  try { writeFileSync(snap.path, snap.content); return true } catch { return false }
}

/**
 * Compose-validate a profile's persisted bundle stack in a throwaway
 * subprocess. Never throws; resolves { ok, skipped, error, rows, bundles }.
 * skipped means the validator itself could not run (missing script or core
 * packages) — the caller decides how to treat that.
 */
async function runComposeCheck(profile) {
  ...
  return new Promise((resolve) => {
    ...
    try {
      child = spawn(process.execPath,
        ['--expose-internals', COMPOSE_CHECK_SCRIPT, '--home', home, '--profile', profile, '--pkgroot', pkgroot],
        { cwd: profileDir, env: { ...process.env }, stdio: ['ignore', 'pipe', 'pipe'] })
    } catch (e) {
      return resolve({ ok: true, skipped: true, error: 'compose-check 无法启动：' + String((e && e.message) || e) })
    }
    let out = ''
    child.stdout.on('data', (d) => { out += d.toString() })
    child.stderr.on('data', (d) => { out += d.toString() })
    const timer = setTimeout(() => { terminateTree(child) }, COMPOSE_CHECK_TIMEOUT)
    child.on('error', (e) => {
      clearTimeout(timer)
      resolve({ ok: true, skipped: true, error: 'compose-check 无法启动：' + String((e && e.message) || e) })
    })
    child.on('close', (code) => {
      clearTimeout(timer)
      ...
      if (code === 0) {
        try {
          ...
          if (j && j.ok === true) return resolve({ ok: true, skipped: false, rows: j.rows, bundles: j.bundles })
        } catch {}
        return resolve({ ok: true, skipped: true, error: 'compose-check 输出异常' })
      }
      let error = String(out || '').trim().slice(-4000)
      try {
        const j = JSON.parse(last || '{}')
        if (j && j.ok === false && j.error) error = j.error
      } catch {}
      resolve({ ok: false, skipped: false, error: error || ('compose-check 退出码 ' + code) })
    })
  })
}

// ------------------------------------------------- bundle fan-out audit
// In-memory npm manifest cache (advisory only; the compose-check is the gate).

const npmManifestCache = new Map()

async function fetchNpmManifest(name) {
  const key = String(name || '').trim()
  if (!key) return null
  if (npmManifestCache.has(key)) return npmManifestCache.get(key)
  let manifest = null
  try {
    const r = await fetch('https://registry.npmjs.org/' + encodeURIComponent(key) + '/latest', {
      redirect: 'follow', signal: AbortSignal.timeout(6000),
    })
    if (r.ok) manifest = await r.json()
  } catch {}
  npmManifestCache.set(key, manifest)
  return manifest
}

/**
 * Walk the target's dependency tree (budgeted BFS) and list every package
 * that ALSO declares `dsh.bundle`. Each such dependency becomes its own
 * profile layer through the official `dsh plugin` reconcile, which is how an
 * aggregate package duplicates loader entry ids and bricks the next boot.
 */
async function auditBundleFanout(source) {
  let root = await fetchNpmManifest(source)
  if (!root) {
    // GitHub-source installs: read the repo's package.json over the raw chain.
    const m = /^github:([^/]+)\/([^/]+?)(?:\.git)?$/.exec(String(source || ''))
    if (m) {
      for (const base of RAW_MIRRORS) {
        try {
          const r = await fetch(base(m[1], m[2], 'package.json'), { redirect: 'follow', signal: AbortSignal.timeout(8000) })
          if (r.ok) { root = await r.json(); break }
        } catch {}
      }
    }
  }
  ...
  while (queue.length > 0 && budget > 0) {
    const level = queue.splice(0, 6)
    budget -= level.length
    const results = await Promise.all(level.map(async (name) => {
      if (seen.has(name)) return null
      seen.add(name)
      const m = await fetchNpmManifest(name)
      if (!m || typeof m !== 'object') return null
      if (m.dsh && m.dsh.bundle) bundles.push(name)
  ...
  return [...new Set(bundles)].slice(0, 12)
}
...
function launchTask(kind, profile, target, label, explicitBin, ctx) {
  ...
  const task = {
    ...
    exitCode: null,
    bin: binInfo.bin,
    // Pre-op manifest snapshot — the rollback source of truth if the post-op
    // compose-check finds the profile would no longer boot.
  ...
  child.on('close', async (code) => {
    if (task.status !== 'running') return
    // The pnpm step is over; the TASK_TIMEOUT only governs pnpm, so stop the
    // timer before the (possibly slow) compose-check runs.
    clearTimeout(task.timer)
    if (code !== 0) {
      finishTask(task, 'failed', code)
      if (kind === 'install') {
    ...
    // 兜底机制：install/update 落地后、热应用之前，先在子进程里用与启动完全
    // 相同的组合逻辑校验新 bundle 栈。失败说明下次启动会崩（重复 loader 条目、
    // 非法配置、包缺失等），立即自动回滚到操作前的 manifest，绝不让 profile
    // 处于无法启动的状态。
    if (kind === 'install' || kind === 'update') {
      const check = await runComposeCheck(profile)
      if (!check.ok) {
        appendLog(task, '\n\n[组合校验失败] ' + (check.error || '未知错误'))
        const restored = restoreProfile(task.snapshot)
        appendLog(task, '\n[自动回滚] ' + (restored
          ? '已恢复操作前的 profile 配置，正在卸载 ' + target + '…'
          : '⚠ 恢复 profile 配置失败！请立即人工检查 ' + profilePath(profile) + '/package.json'))
        if (restored) {
          const rbBin = locateDshCli()
          if (rbBin) await runProcess(rbBin, ['plugin', '--profile', profile, 'remove', target, '-w'], profilePath(profile), 90000)
        }
        appendLog(task, '\n[回滚完成] 本次' + (kind === 'install' ? '安装' : '更新') + '已取消，profile 恢复原状，可安全重启。'
          + (restored ? '' : '\n' + REPAIR_HINT + profile))
        finishTask(task, 'rolledback', 0)
        recordApply([{ mode: 'rolledback', label, error: check.error, restored }], task.id)
        return
      }
      appendLog(task, '\n[组合校验] ✓ 新的 bundle 组合可正常启动')
      if (kind === 'update') advanceBaseline(profile, target)
    }
    finishTask(task, 'done', 0)
...
async function runBootUpdater(ctx) {
  try {
    ...
    const applyUpdate = async (profile, depKey, label) => {
      ...
      // Same boot-safety gate as interactive installs: never leave a profile
      // that cannot compose (auto-update must not brick the next boot).
      const check = await runComposeCheck(profile)
      if (!check.ok) {
        const restored = restoreProfile(snap)
        console.error('[dsh-plugin-market] boot auto-update rolled back ' + label + ': ' + (check.error || '') + (restored ? '' : ' (manifest restore failed!)'))
        applies.push({ mode: 'rolledback', label, error: check.error, restored })
        if (restored) {
          const rbBin = locateDshCli()
          if (rbBin) await runProcess(rbBin, ['plugin', '--profile', profile, 'remove', depKey, '-w'], profilePath(profile), 90000)
        }
        return false
      }
...
export async function applyPluginChange(ctx, opts) {
  ...
  try {
    ...
    // nothing persists and nothing double-mounts. A mount whose plugin fails
    // to start must not leave a dead entry in the running tree.
    const name = resolveInstalledName(profile, target) || String(target || '')
    const createdId = await ctx.loader.create({ name, config: {} })
    const created = ctx.loader.resolve(createdId)
    try {
      // create resolves before the entry starts: wait briefly for the fiber,
      // then give it a grace period. Only a definitively failed fiber (or one
      // that never materialized) is disposed — slow activators stay mounted.
      let fiber = created && created.fiber
      for (let i = 0; i < 20 && !fiber; i++) { await delay(100); fiber = created.fiber }
      if (fiber) await Promise.race([fiber.await().catch(() => {}), delay(4000)])
      const finalFiber = created && created.fiber
      if (!finalFiber || finalFiber.state === 3) {
        try { await created._dispose() } catch {}
        return { mode: 'restart', label: labelName }
      }
    } catch (e) {
      try { await created._dispose() } catch {}
      console.error('[dsh-plugin-market] hot-mount failed for ' + labelName + ':', String((e && e.message) || e))
      return { mode: 'restart', label: labelName }
    }
    return { mode: 'reload', label: labelName }
  } catch (e) {
      ...
      try {
        ...
        if (method === 'install' || method === 'update' || method === 'uninstall') {
          ...
          if (method === 'install' && profile === 'web' && !body.skipCheck) {
            ...
            // Bundle fan-out audit: dependencies that also declare dsh.bundle
            // become profile layers through the official reconcile and
            // routinely duplicate loader entries (the dsh-web-ui crash). The
            // post-install compose-check + auto-rollback still guards the
            // install, but this requires an explicit confirmation first.
            if (!body.confirmFanout) {
              const fanout = await auditBundleFanout(target)
              if (fanout.length > 0) {
                return writeJson(res, 200, {
                  ok: false,
                  refused: true,
                  fanout: true,
                  bundles: fanout,
                  output: '该插件的依赖里有 ' + fanout.length + ' 个包也声明了 dsh.bundle，会被 `dsh plugin` 自动挂载为 profile 层：'
                    + fanout.join('、') + '。这类"全家桶"插件极易产生重复 loader 条目（duplicate loader entry id）导致下次启动崩溃。'
                    + '如确认继续，请勾选"我了解风险"；安装后仍会自动做启动组合校验，校验失败会自动回滚。',
                })
              }
            }
          }
