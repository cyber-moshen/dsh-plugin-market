...
async function annotateInstalled(plugins) {
  ...
  // Per-profile set of row ids currently disabled by our own patch entries.
  const profileDisabled = new Map()
  for (const profile of profiles) profileDisabled.set(profile, profileDisabledIds(profile))
  const out = []
  for (const p of plugins) {
    ...
    const rows = bundleRowIds(join(profilePath(entry.profile), 'node_modules', entry.depKey))
    const disabled = profileDisabled.get(entry.profile) || new Set()
    out.push({
      ...p,
      inst: {
        ...
        enabled: rows.length === 0 || !rows.some((id) => disabled.has(id)),
      },
    })
  }
  return out
...
/**
 * Row ids a bundle inserts, read from its cordis.patch.yml (insert blocks
 * only — update/remove ops are not "the plugin's rows").
 */
function bundleRowIds(pkgDir) {
  const f = join(pkgDir, 'cordis.patch.yml')
  ...
  for (const raw of lines) {
    ...
    if (content === 'insert:' || content === '- insert:') { inInsert = true; insertIndent = indent; continue }
    if (inInsert) {
      if (indent <= insertIndent) inInsert = false
      else {
        const m = /^- id:\s*(\S+)/.exec(content)
        if (m) ids.push(m[1])
      }
    }
  }
...
/** Row ids disabled by `disabled: true` entries in the profile's own patch layer. */
function profileDisabledIds(profile) {
  const f = join(profilePath(profile), 'cordis.patch.yml')
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (m && i + 1 < lines.length && /^\s+disabled:\s*true\s*$/.test(lines[i + 1])) ids.add(m[1])
  }
  return ids
}

/**
 * Enable or disable a bundle by editing the profile's own cordis.patch.yml
 * (the user layer applied after every bundle layer): disable appends
 * `- id: <row>` / `  disabled: true` pairs for the bundle's inserted rows,
 * enable removes exactly those pairs. Takes effect on the next boot.
 */
function applyToggle(profile, rows, wantEnabled) {
  const patchFile = join(profilePath(profile), 'cordis.patch.yml')
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (m && rows.includes(m[1]) && i + 1 < lines.length && /^\s+disabled:\s*true\s*$/.test(lines[i + 1])) {
      i++ // drop this disable pair
      continue
    }
    if (!wantEnabled && line.trim() === '[]') {
      out.push(...rows.flatMap((id) => [`- id: ${id}`, '  disabled: true']))
      replacedEmpty = true
      continue
    }
    out.push(line)
  }
  if (!wantEnabled && !replacedEmpty) {
    const present = new Set()
    for (let i = 0; i < out.length; i++) {
      const m = /^\s*- id:\s*(\S+)\s*$/.exec(out[i])
      if (m && i + 1 < out.length && /^\s+disabled:\s*true\s*$/.test(out[i + 1])) present.add(m[1])
    }
    const missing = rows.filter((id) => !present.has(id))
    if (missing.length) out.push('', ...missing.flatMap((id) => [`- id: ${id}`, '  disabled: true']))
  }
  if (wantEnabled && !out.some((l) => /^\s*- /.test(l) && l.trim() !== '')) {
    if (!out.some((l) => l.trim() === '[]')) out.push('[]')
  }
  const next = out.join('\n').replace(/\s+$/, '') + '\n'
  writeFileSync(patchFile, next)
  return { ok: true, rows, enabled: wantEnabled, restart: true }
}
    ...
    handler: async (req, res) => {
      try {
        ...
        if (method === 'toggle') {
          ...
          const depKey = String(body.depKey || '').trim()
          if (!depKey) return writeJson(res, 400, { ok: false, error: '缺少参数' })
          const pkgDir = join(profilePath(profile), 'node_modules', depKey)
          const rows = bundleRowIds(pkgDir)
          if (rows.length === 0) {
            return writeJson(res, 200, { ok: false, error: '无法定位该插件的补丁行（未安装或不是 bundle）' })
          }
          const wantEnabled = body.enabled === true
          return writeJson(res, 200, applyToggle(profile, rows, wantEnabled))
        }
        if (method === 'op') {
