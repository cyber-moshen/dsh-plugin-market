...
// Settings → Plugins.
//
// Search row: [search] [过滤] [设置]. The filter popover sets installed-state
// filtering (all/installed/not) and sort order (default/stars/latest commit).
// The settings modal stores a GitHub token (avoids API rate limits) and toggles
// startup auto-update. Cards show tags (click-to-search), stars, last-commit
// freshness and an install/update action; the GitHub icon button jumps to the
// repo. A frame-wide overlay (shell.overlay) shows a "plugins auto-updated —
// restart to apply" popup on any page after a startup auto-update. Icons are
// GitHub's octicons (MIT) inlined as SVG paths.
...
window.__ModuleLoader__.load({ id: 'dsh-plugin-market', factory: (require) => {
  ...
  try {
    ...
    else if (nl.toLowerCase().startsWith('ja')) LOCALE = 'ja'
  } catch (e) {}
  ...
  const STR = {
    ...
    ja: {
      search: 'プラグインを検索（名前/説明/タグ/作者）…', cfgBtn: '設定', close: '閉じる',
      loading: 'プラグイン一覧を読み込み中…', noMatch: '一致するプラグインがありません', fetchFail: '取得失敗',
      install: 'インストール', update: '更新', uninstall: 'アンインストール', execute: '実行', cancel: 'キャンセル',
      min: '最小化', kill: '終了', liveChip: 'プラグインタスク',
      installOk: 'インストールしました。Webサービスを再起動してください', updateOk: '更新しました。Webサービスを再起動してください', uninstallOk: 'アンインストールしました。Webサービスを再起動してください', opFailed: '操作に失敗しました',
      running: '実行中…（pnpmは時間がかかる場合があります）', submit: '送信中…',
      stDone: '完了', stFailed: '失敗', stKilled: '終了', stTimeout: 'タイムアウト',
      stBusy: 'タスク実行中', stRefused: '拒否',
      elapsed: '{s}秒経過（{t}秒で自動終了）',
      enrich: 'スター/コミット情報を取得中 {d}/{t}…',
      stars: 'スター', commit: '最終コミット', maintain: 'メンテナンス',
      notInstalled: '未インストール', installed: 'インストール済み', toV: '→ v{v}',
      instLabel: 'インストール状態', instAll: 'すべて', instInstalled: 'インストール済み', instNot: '未インストール',
      sortLabel: '並び順', sortStarsAsc: 'スター昇順', sortStarsDesc: 'スター降順', sortCommitAsc: 'コミット昇順', sortCommitDesc: 'コミット降順',
      maintLabel: 'メンテナンス', maintAll: 'すべて', maintActive: '活発', maintStale: 'やや古い', maintDead: '放置の可能性', maintUnknown: '不明',
      hint: 'インストール/更新後はWebサービスの再起動が必要です。',
      skipCheck: '完全アプリ型チェックをスキップ（自己責任：Web起動を壊す可能性があります）',
      noCmd: '（公式インストールコマンドなし）', maintainNew: '活発', maintainMid: 'やや古い', maintainOld: '放置の可能性',
      unknown: '不明', github: 'GitHub', ok: 'OK',
      noticeTitle: 'プラグインを自動更新しました', noticeText: '{n}個のプラグインを自動更新しました：{list}。Webサービスを再起動してください。',
    },
  }
  ...
  // Config-page strings: fixed per language chosen IN the settings modal
  // (labels never follow the app locale — the user picks them explicitly).
  const CFG_STR = {
    zh: { title: '插件市场设置', langLabel: '界面语言', token: 'GitHub Token（可选，避免 API 限流）', save: '保存', autoUpdate: '启动时自动更新已安装插件', hint: '开启后，每次启动 Web 服务会检测已安装插件，有新版本会自动更新；更新完成后会在首页弹窗提示重启生效。', close: '关闭' },
    en: { title: 'Plugin market settings', langLabel: 'Language', token: 'GitHub Token (optional, avoids rate limits)', save: 'Save', autoUpdate: 'Auto-update installed plugins on startup', hint: 'When enabled, the web server checks installed plugins on startup and updates them automatically; a popup on the home page asks you to restart once done.', close: 'Close' },
    ja: { title: 'プラグインマーケット設定', langLabel: '言語', token: 'GitHubトークン（任意、API制限回避）', save: '保存', autoUpdate: '起動時にインストール済みプラグインを自動更新', hint: '有効にすると、Webサービスの起動時にインストール済みプラグインを確認し、新しいバージョンがあれば自動更新します。完了後、ホームページに再起動を促すポップアップが表示されます。', close: '閉じる' },
  }
...
.mkc-row1{display:flex;gap:8px;align-items:center;padding-block:12px}
.mkc-search{flex:1;background:transparent;border:1px solid var(--dsw-alias-border-l2);border-radius:10px;color:var(--dsw-alias-label-primary);font:inherit;font-size:14px;padding:9px 12px;min-width:0;height:40px;box-sizing:border-box}
.mkc-search::placeholder{color:var(--dsw-alias-label-tertiary)}
.mkc-row1 .mkc-btn{height:40px;padding:0 16px;display:inline-flex;align-items:center}
.mkc-filterbar{display:flex;flex-direction:column;gap:8px;padding:10px 0 12px;border-bottom:1px solid var(--dsw-alias-border-l2);margin-bottom:2px}
.mkc-pop-sec{display:flex;align-items:center;gap:8px;flex-wrap:wrap}
.mkc-pop-label{font-size:12px;color:var(--dsw-alias-label-tertiary);min-width:56px}
.mkc-chip{font-size:12.5px;color:var(--dsw-alias-label-secondary);background:none;white-space:nowrap;border:1px solid var(--dsw-alias-border-l2);border-radius:999px;padding:3px 12px;cursor:pointer}
...
.mkc-enrich{font-size:12.5px;color:var(--dsw-alias-label-tertiary);padding:4px 2px 0}
...
.mkc-cfgrow{display:flex;gap:8px;align-items:center;margin-bottom:10px}
...
.mkc-notice{position:fixed;top:16px;left:50%;transform:translateX(-50%);z-index:1300;display:flex;align-items:center;gap:14px;max-width:min(640px,92vw);background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l2);border-radius:12px;padding:12px 16px;box-shadow:0 12px 36px rgba(0,0,0,.28)}
.mkc-notice-title{font-size:14px;font-weight:650;color:var(--dsw-alias-label-primary)}
.mkc-notice-text{font-size:12.5px;color:var(--dsw-alias-label-secondary);margin-top:2px;overflow-wrap:break-word}
`
  ...
  function RestartNotice() {
    ...
    useEffect(() => {
      ...
      api('notice').then((r) => {
        ...
        let seen = ''
        try { seen = localStorage.getItem('mkcNoticeDismissed') || '' } catch (e) {}
    ...
    const dismiss = () => {
      try { localStorage.setItem('mkcNoticeDismissed', String(notice.at)) } catch (e) {}
      setNotice(null)
    }
    return h('div', { className: 'mkc-notice' },
      h('div', null,
        h('div', { className: 'mkc-notice-title' }, t('noticeTitle')),
        h('div', { className: 'mkc-notice-text' }, fmt('noticeText', { n: notice.updated.length, list: notice.updated.join('、') })),
      ),
      h('button', { className: 'mkc-btn mkc-btn-primary', onClick: dismiss }, t('ok')),
    )
  }
  ...
  function MarketCardPanel() {
    ...
    const [cfgLang, setCfgLang] = useState((() => {
      try {
        const v = localStorage.getItem('mkcCfgLang')
    ...
    const loadList = (alive) => {
      api('list', { lang: LOCALE }).then((r) => {
        ...
        const en = r.enriched || {}
        if (en.running && alive) setTimeout(() => loadList(alive), 4000)
      }).catch((e) => {
        if (!alive) return
    ...
    const maintOfP = (p) => maintainOf(p.pushedAt) || 'unknown'
    const filtered = (data.plugins || []).filter((p) => {
      ...
      if (maintFilter !== 'all' && maintOfP(p) !== maintFilter) return false
    ...
    const ct = CFG_STR[cfgLang] || CFG_STR.en
    const setCfgLangPersist = (v) => { setCfgLang(v); try { localStorage.setItem('mkcCfgLang', v) } catch (e) {} }
    const cfgModalEl = cfgModal ? h('div', { className: 'mkc-modal-bg', onClick: () => setCfgModal(false) },
      h('div', { className: 'mkc-modal', onClick: (e) => e.stopPropagation() },
        h('h4', null, ct.title),
        h('div', { className: 'mkc-cfgrow', style: { flexWrap: 'wrap' } },
          h('span', { className: 'mkc-pop-label' }, ct.langLabel),
          [['zh', '中文'], ['en', 'English'], ['ja', '日本語']].map(([v, label]) =>
            h('button', { key: v, className: 'mkc-chip' + (cfgLang === v ? ' mkc-chip-on' : ''), onClick: () => setCfgLangPersist(v) }, label)),
        ),
        h('div', { className: 'mkc-cfgrow' },
          h('input', { className: 'mkc-bin-input', type: 'password', placeholder: ct.token + (cfg.tokenSet ? ' ✓' : ''), value: tokenInput, onChange: (e) => setTokenInput(e.target.value) }),
          h('button', { className: 'mkc-btn mkc-btn-primary', onClick: saveCfg }, ct.save),
        ),
        h('label', { className: 'mkc-skipcheck', style: { marginTop: 0 } },
          h('input', { type: 'checkbox', checked: autoUpdate || cfg.autoUpdate, onChange: (e) => toggleAutoUpdate(e.target.checked) }),
          h('span', null, ct.autoUpdate),
        ),
        h('div', { className: 'mkc-hint' }, ct.hint),
        h('div', { className: 'mkc-cmdrow' }, h('button', { className: 'mkc-btn', onClick: () => setCfgModal(false) }, ct.close)),
      )) : null
    ...
    return h('div', { className: 'mkc' },
      ...
      h('div', { className: 'mkc-finder' },
        h('div', { className: 'mkc-row1' },
          ...
          h('button', { className: 'mkc-btn', onClick: () => setCfgModal(true) }, t('cfgBtn')),
          liveChip,
        ),
        h('div', { className: 'mkc-filterbar' },
          h('div', { className: 'mkc-pop-sec' },
            h('span', { className: 'mkc-pop-label' }, t('instLabel')),
            [['all', t('instAll')], ['installed', t('instInstalled')], ['not', t('instNot')]].map(([v, label]) =>
              h('button', { key: v, className: 'mkc-chip' + (instFilter === v ? ' mkc-chip-on' : ''), onClick: () => setInstFilter(v) }, label)),
          ),
          h('div', { className: 'mkc-pop-sec' },
            h('span', { className: 'mkc-pop-label' }, t('sortLabel')),
            [['stars-asc', t('sortStarsAsc')], ['stars-desc', t('sortStarsDesc')], ['commit-asc', t('sortCommitAsc')], ['commit-desc', t('sortCommitDesc')]].map(([v, label]) =>
              h('button', { key: v, className: 'mkc-chip' + (sortBy === v ? ' mkc-chip-on' : ''), onClick: () => setSortBy(v) }, label)),
          ),
          h('div', { className: 'mkc-pop-sec' },
            h('span', { className: 'mkc-pop-label' }, t('maintLabel')),
            [['all', t('maintAll')], ['ok', t('maintActive')], ['mid', t('maintStale')], ['old', t('maintDead')], ['unknown', t('maintUnknown')]].map(([v, label]) =>
              h('button', { key: v, className: 'mkc-chip' + (maintFilter === v ? ' mkc-chip-on' : ''), onClick: () => setMaintFilter(v) }, label)),
          ),
        ),
      ),
      ...
      data.phase === 'ready' ? h('div', null,
        ...
        sorted.length === 0 ? h('div', { className: 'mkc-hint', style: { padding: '16px 2px' } }, t('noMatch')) : null,
      ) : null,
    )
  }
