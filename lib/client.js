...
`
  ...
  function UpdateToast() {
    ...
    const restartCount = notice.entries.length - hotCount
    const title = notice.mode === 'reload'
      ? t('noticeHotTitle')
      : (notice.mode === 'restart' ? t('noticeRestartTitle') : t('noticeMixedTitle'))
    const text = notice.mode === 'reload'
  ...
  function WorkshopPanel() {
    ...
    const settleTask = (opId, reload) => {
      setTask((prev) => prev && prev.opId === opId
        ? { ...prev, phase: 'done', status: 'done', ok: true, reload }
        : prev)
      refreshCatalog(true)
