...
window.__ModuleLoader__.load({ id: 'dsh-plugin-market', factory: (require) => {
  ...
  const STR = {
    zh: {
      ...
      settingsTitle: '插件工坊设置', token: 'GitHub Token（可选，避免 API 限流）', save: '保存', autoUpdate: '启动时自动更新已安装插件',
    ...
    en: {
      ...
      settingsTitle: 'Plugin Workshop settings', token: 'GitHub Token (optional, avoids rate limits)', save: 'Save', autoUpdate: 'Auto-update installed plugins on startup',
    ...
    const cfgModal = cfgOpen ? h('div', { className: 'wsp-modal-bg', onClick: () => setCfgOpen(false) },
      h('div', { className: 'wsp-modal', onClick: (e) => e.stopPropagation() },
        ...
        h('div', { className: 'wsp-cfgrow' },
          h('input', { className: 'wsp-input', type: 'password', placeholder: t('token') + (cfg.tokenSet ? ' ✓' : ''), value: tokenInput, onChange: (e) => setTokenInput(e.target.value) }),
