...
window.__ModuleLoader__.load({ id: 'dsh-plugin-market', factory: (require) => {
  ...
  const STR = {
    zh: {
      search: '搜索插件名 / 描述 / 标签 / 作者…', filterBtn: '过滤', cfgBtn: '设置', close: '关闭',
      ...
      instAll: '全部', instInstalled: '已安装', instNot: '未安装',
      sortDefault: '默认排序', sortStars: '星标最多', sortCommit: '最近提交（新在前）',
      token: 'GitHub Token（可选，避免 API 限流）', save: '保存', autoUpdate: '启动时自动更新已安装插件',
      autoUpdateHint: '开启后，每次启动 Web 服务会检测已安装插件，有新版本会自动更新；更新完成后会在首页弹窗提示重启生效。',
      settingsTitle: '插件市场设置',
    ...
    en: {
      search: 'Search name / description / tag / author…', filterBtn: 'Filter', cfgBtn: 'Settings', close: 'Close',
      ...
      instAll: 'All', instInstalled: 'Installed', instNot: 'Not installed',
      sortDefault: 'Default', sortStars: 'Most stars', sortCommit: 'Latest commit',
      token: 'GitHub Token (optional, avoids rate limits)', save: 'Save', autoUpdate: 'Auto-update installed plugins on startup',
      ...
      settingsTitle: 'Plugin market settings',
...
.mkc-pop{display:flex;flex-direction:column;gap:10px;padding:12px;border:1px solid var(--dsw-alias-border-l2);border-radius:10px;background:var(--dsw-alias-bg-layer-3);margin-bottom:8px}
...
`
  ...
  function MarketCardPanel() {
    ...
    else if (sortBy === 'commit') sorted.sort((a, b) => new Date(b.pushedAt || 0).getTime() - new Date(a.pushedAt || 0).getTime())
    ...
    const cfgModalEl = cfgModal ? h('div', { className: 'mkc-modal-bg', onClick: () => setCfgModal(false) },
      h('div', { className: 'mkc-modal', onClick: (e) => e.stopPropagation() },
        ...
        h('div', { className: 'mkc-cfgrow' },
          ...
          h('button', { className: 'mkc-btn mkc-btn-primary', onClick: saveCfg }, t('save')),
        ),
        ...
        h('div', { className: 'mkc-hint' }, t('autoUpdateHint')),
        h('div', { className: 'mkc-cmdrow' }, h('button', { className: 'mkc-btn', onClick: () => setCfgModal(false) }, t('close'))),
      )) : null
    ...
    return h('div', { className: 'mkc' },
      ...
      h('div', { className: 'mkc-finder' },
        h('div', { className: 'mkc-row1' },
          ...
          h('button', { className: 'mkc-btn' + (filterPop ? ' mkc-btn-primary' : ''), onClick: () => setFilterPop(!filterPop) }, t('filterBtn')),
          h('button', { className: 'mkc-btn', onClick: () => setCfgModal(true) }, t('cfgBtn')),
          liveChip,
          h('span', { className: 'mkc-count' }, sorted.length + ' / ' + (data.plugins || []).length),
        ),
        filterPop ? h('div', { className: 'mkc-pop' },
          h('div', { className: 'mkc-pop-sec' },
            h('span', { className: 'mkc-pop-label' }, t('instInstalled')),
          ...
          h('div', { className: 'mkc-pop-sec' },
            h('span', { className: 'mkc-pop-label' }, t('sortDefault')),
            [['default', t('sortDefault')], ['stars', t('sortStars')], ['commit', t('sortCommit')]].map(([v, label]) =>
