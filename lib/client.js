...
window.__ModuleLoader__.load({ id: '@cyber-moshen/dsh-plugin-market', factory: (require) => {
  ...
  const STR = {
    zh: {
      ...
      notInstalled: '未安装', installed: '已安装', disabled: '已禁用', enable: '启用', disable: '禁用', toV: '→ v{v}',
      enabledDone: '已启用，重启 Web 服务后生效', disabledDone: '已禁用，重启 Web 服务后生效', toggleDone: '已修改，重启 Web 服务后生效',
    ...
    en: {
      ...
      notInstalled: 'Not installed', installed: 'Installed', disabled: 'Disabled', enable: 'Enable', disable: 'Disable', toV: '→ v{v}',
      enabledDone: 'Enabled — restart the web server to apply', disabledDone: 'Disabled — restart the web server to apply', toggleDone: 'Changed — restart the web server to apply',
...
`
  ...
  function WorkshopPanel() {
    ...
    /** Enable/disable an installed bundle via the profile's own patch layer (no pnpm involved). */
    const runToggle = (p) => {
      const inst = p.inst || {}
      const wantEnabled = !(inst.enabled === false)
      rpc('toggle', { profile: inst.profile, depKey: inst.depKey, enabled: wantEnabled }).then((r) => {
        if (r && r.ok) {
          setTask({ kind: 'toggle', label: p.name, phase: 'done', status: 'done', ok: true, exitCode: 0, output: wantEnabled ? t('enabledDone') : t('disabledDone') })
          refreshCatalog(true)
        } else {
          setTask({ kind: 'toggle', label: p.name, phase: 'done', status: 'failed', ok: false, exitCode: 1, output: String((r && (r.output || r.error)) || t('opFailed')) })
        }
      }).catch((e) => {
        setTask({ kind: 'toggle', label: p.name, phase: 'done', status: 'failed', ok: false, exitCode: 1, output: String((e && e.message) || e) })
      })
    }
    ...
    const taskTitle = (tk) => {
      const verb = { install: t('install'), update: t('update'), uninstall: t('uninstall'), toggle: '' }[tk.kind]
      return verb ? verb + ' ' + tk.label : tk.label
    }
      ...
      h('div', { className: 'wsp-modal', onClick: (e) => e.stopPropagation() },
        ...
        task.phase === 'done' ? h('div', null,
          h('div', { style: { fontSize: 13, fontWeight: 600, color: task.ok ? 'var(--dsw-alias-state-success-primary)' : 'var(--dsw-alias-label-error)' } },
            task.ok
              ? ({ install: t('installOk'), update: t('updateOk'), uninstall: t('uninstallOk'), toggle: t('toggleDone') })[task.kind] || t('opFailed')
              ...
              h('div', { className: 'wsp-foot' },
                inst.state === 'none'
                  ...
                  : h('span', { className: 'wsp-state wsp-state-on' },
                      t('installed') + (inst.version ? ' v' + inst.version : '') + (inst.enabled === false ? ' · ' + t('disabled') : '')),
                ...
                inst.state === 'none'
                  ...
                  : h('div', { className: 'wsp-foot-actions' },
                      inst.state === 'update'
                        ...
                        : null,
                      h('button', { className: 'wsp-btn', onClick: () => runToggle(p) }, inst.enabled === false ? t('enable') : t('disable')),
