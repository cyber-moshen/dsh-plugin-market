...
window.__ModuleLoader__.load({ id: '@cyber-moshen/dsh-plugin-market', factory: (require) => {
  ...
  const STR = {
    zh: {
      ...
      workshop: '插件工坊', selfUpdateTip: '更新插件工坊自身（npm 版本）', selfUninstallTip: '卸载插件工坊，重启后消失',
    ...
    en: {
      ...
      workshop: 'Plugin Workshop', selfUpdateTip: 'Update the workshop itself (npm version)', selfUninstallTip: 'Uninstall the workshop — gone after restart',
      ...
      h('div', { className: 'wsp-finder' },
        ...
        h('div', { className: 'wsp-filters' },
          ...
          h('div', { className: 'wsp-self' },
            h('button', { className: 'wsp-btn', title: t('selfUpdateTip'), onClick: () => askOperation('update', SELF_PACKAGE, t('workshop'), 'web') }, t('update')),
            h('button', { className: 'wsp-btn wsp-btn-danger', title: t('selfUninstallTip'), onClick: () => askOperation('uninstall', SELF_PACKAGE, t('workshop'), 'web') }, t('uninstall')),
          ),
        ),
      ),
