...
// Settings → Plugins. Cards show GitHub stars, last-commit freshness (colored
// maintenance signal) and an install/uninstall action; the top-right GitHub
// icon button jumps to the repo (no in-app README viewer — the README opens on
// GitHub itself). Icons are GitHub's own octicons (MIT) inlined as SVG paths.
// Install/uninstall run as Host background ops with a confirm → progress(log)
// → done modal. Talks to the Host half over /api/dsh-plugin-market.
// Loaded by the web plugin loader (window.__ModuleLoader__); the factory id
// MUST equal the package name (the boot graph row id).
window.__ModuleLoader__.load({ id: 'dsh-plugin-market', factory: (require) => {
  ...
  function repoParts(url) {
    ...
    return m ? { owner: m[1], repo: m[2].replace(/\.git$/, '') } : { owner: '', repo: '' }
  }
  ...
  function installedPkgName(plugin, installed) {
    ...
    const repo = (repoParts(plugin.url).repo) || String(plugin.name || '')
  ...
  const STR = {
    zh: {
      search: '搜索插件名 / 描述 / 作者…', all: '全部', instFilter: '已安装', close: '关闭',
      loading: '加载插件目录…', noMatch: '没有匹配的插件', fetchFail: '抓取失败',
      install: '安装', uninstall: '卸载', execute: '执行', cancel: '取消', min: '最小化到后台', kill: '终止任务',
      liveChip: '插件任务',
      ...
      notInstalled: '未安装',
      envOk: '环境正常', envBad: '环境异常', dshMiss: 'dsh CLI 未定位，请填写路径或检查安装',
    ...
    en: {
      search: 'Search name / description / author…', all: 'All', instFilter: 'Installed', close: 'Close',
      loading: 'Loading plugin directory…', noMatch: 'No matching plugins', fetchFail: 'Fetch failed',
      install: 'Install', uninstall: 'Uninstall', execute: 'Run', cancel: 'Cancel', min: 'Minimize', kill: 'Kill',
      ...
      envOk: 'Environment OK', envBad: 'Environment issue', dshMiss: 'dsh CLI not found — fill the path or fix the install',
      ...
      hint: 'Restart the web server after installs.',
...
.mkc-env{font-family:ui-monospace,"SF Mono","Cascadia Code",Menlo,monospace;font-size:12px;color:var(--dsw-alias-label-tertiary);margin-bottom:10px;white-space:pre-wrap}
.mkc-env-bad{color:var(--dsw-alias-label-error)}
.mkc-bin-row{display:flex;gap:8px;align-items:center;margin-bottom:10px}
.mkc-bin-input{flex:1;background:transparent;border:1px solid var(--dsw-alias-border-l2);border-radius:8px;color:var(--dsw-alias-label-primary);font-family:ui-monospace,monospace;font-size:13px;padding:6px 10px}
...
.mkc-chips{display:flex;flex-wrap:wrap;gap:8px;padding-bottom:12px;border-bottom:1px solid var(--dsw-alias-border-l2)}
...
.mkc-badge-cat{background:var(--dsw-alias-bg-module-platform);color:var(--dsw-alias-label-secondary)}
.mkc-card-by{font-family:ui-monospace,monospace;font-size:12px;color:var(--dsw-alias-label-tertiary)}
...
.mkc-card-foot{display:flex;align-items:center;gap:8px;margin-top:auto;padding-top:4px}
...
`
  ...
  function MarketCardPanel() {
    const [data, setData] = useState({ phase: 'loading', plugins: [], installed: null, error: null, enriched: null })
    ...
    useEffect(() => {
      ...
      const load = () => {
        api('list', { lang: LOCALE }).then((r) => {
          ...
          setData((d) => ({ ...d, phase: 'ready', plugins: r.plugins || [], categories: r.categories || {}, source: r.source || 'repo', enriched: r.enriched || null }))
          loadInstalled(r.plugins || [])
          const en = r.enriched || {}
          if (en.running && alive) setTimeout(load, 4000)
        }).catch((e) => {
          if (!alive) return
    ...
    const catLabel = (id) => {
      const c = catLabels[id]
      if (c) return c[LOCALE] || c.en || id
      return id
    }

    const chips = [{ id: 'all', label: t('all'), count: (data.plugins || []).length }]
    const seen = []
    for (const p of data.plugins || []) {
      if (p.category && !seen.includes(p.category)) seen.push(p.category)
    }
    for (const id of seen) {
      chips.push({ id, label: catLabel(id), count: (data.plugins || []).filter((p) => p.category === id).length })
    }
    ...
    const filtered = (data.plugins || []).filter((p) => {
      ...
      if (q && !((p.name || '').toLowerCase().includes(q) || (p.desc || '').toLowerCase().includes(q) || (p.owner || '').toLowerCase().includes(q))) return false
      return true
    })
    ...
    return h('div', { className: 'mkc' },
      ...
      h('div', { className: 'mkc-finder' },
        ...
        h('div', { className: 'mkc-chips' },
          chips.map((c) => h('button', {
            key: c.id,
            className: 'mkc-chip' + (cat === c.id && !showInstalled ? ' mkc-chip-on' : ''),
            onClick: () => { setCat(c.id); setShowInstalled(false) },
          }, c.label, ' ', h('small', null, c.count))),
          h('button', {
            ...
            return h('div', { key: p.url, className: 'mkc-card' },
              h('div', { className: 'mkc-card-top' },
                h('div', { className: 'mkc-card-title' },
                  p.name,
                  p.category ? h('span', { className: 'mkc-badge mkc-badge-cat' }, catLabel(p.category)) : null,
              ...
              h('div', { className: 'mkc-card-foot' },
                h('span', { className: 'mkc-state ' + (inst ? 'mkc-state-on' : 'mkc-state-off') }, inst ? t('instFilter') : t('notInstalled')),
                h('span', { className: 'mkc-hint', style: { marginLeft: 'auto' } }, t('hint')),
                inst
                  ? h('button', { className: 'mkc-btn mkc-btn-danger', onClick: () => runOp('uninstall', installedPkgName(p, data.installed) || p.name, p.name, p.profile) }, t('uninstall'))
