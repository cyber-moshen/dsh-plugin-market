...
// install/update action. The settings modal picks the workshop language
// (中文/English/日本語 — fixed per choice), stores a GitHub token and toggles
// startup auto-update. A frame-wide overlay shows a "plugins auto-updated —
// restart to apply" popup on any page after a boot-time auto-update.
// Icons are GitHub's octicons (MIT), inlined as SVG paths.
...
window.__ModuleLoader__.load({ id: 'dsh-plugin-market', factory: (require) => {
  ...
  const STR = {
    ...
    ja: {
      ...
      loading: 'プラグイン一覧を読み込み中…', noMatch: '一致するプラグインがありません', fetchFail: '取得失敗', badResponse: 'サービスが正しく応答しませんでした（not found の場合はブラウザを強制更新し、Webサーバーを再起動してください）',
  ...
  // Workshop-page strings: fixed per language chosen inside the settings
  // modal; the labels below never follow the app locale.
  const CFG_STR = {
    zh: { title: '插件工坊设置', langLabel: '界面语言', token: 'GitHub Token（可选，避免 API 限流）', save: '保存', autoUpdate: '启动时自动更新已安装插件', hint: '开启后，每次启动 Web 服务会检测已安装插件，有新版本会自动更新；更新完成后会在首页弹窗提示重启生效。', close: '关闭' },
    en: { title: 'Plugin Workshop settings', langLabel: 'Language', token: 'GitHub Token (optional, avoids rate limits)', save: 'Save', autoUpdate: 'Auto-update installed plugins on startup', hint: 'When enabled, the web server checks installed plugins on startup and updates them automatically; a popup on the home page asks you to restart once done.', close: 'Close' },
    ja: { title: 'プラグイン工房の設定', langLabel: '言語', token: 'GitHubトークン（任意、API制限回避）', save: '保存', autoUpdate: '起動時にインストール済みプラグインを自動更新', hint: '有効にすると、Webサービスの起動時にインストール済みプラグインを確認し、新しいバージョンがあれば自動更新します。完了後、ホームページに再起動を促すポップアップが表示されます。', close: '閉じる' },
  }
...
`
  ...
  function WorkshopPanel() {
    ...
    const [cfgLang, setCfgLang] = useState((() => {
      try {
        const v = localStorage.getItem('wspCfgLang')
    ...
    const persistConfig = () => {
      rpc('config', { githubToken: tokenInput }).then((r) => {
    ...
    const setCfgLangPersist = (v) => { setCfgLang(v); try { localStorage.setItem('wspCfgLang', v) } catch (e) {} }
    const cfgModal = cfgOpen ? h('div', { className: 'wsp-modal-bg', onClick: () => setCfgOpen(false) },
      h('div', { className: 'wsp-modal', onClick: (e) => e.stopPropagation() },
        h('h4', null, ct.title),
        h('div', { className: 'wsp-cfgrow', style: { flexWrap: 'wrap' } },
          h('span', { className: 'wsp-flabel' }, ct.langLabel),
          [['zh', '中文'], ['en', 'English'], ['ja', '日本語']].map(([v, label]) =>
            h('button', { key: v, className: 'wsp-chip' + (cfgLang === v ? ' wsp-chip-on' : ''), onClick: () => setCfgLangPersist(v) }, label)),
        ),
        h('div', { className: 'wsp-cfgrow' },
          h('input', { className: 'wsp-input', type: 'password', placeholder: ct.token + (cfg.tokenSet ? ' ✓' : ''), value: tokenInput, onChange: (e) => setTokenInput(e.target.value) }),
          h('button', { className: 'wsp-btn wsp-btn-primary', onClick: persistConfig }, ct.save),
        ),
        ...
        h('div', { className: 'wsp-hint' }, ct.hint),
        h('div', { className: 'wsp-cmdrow' }, h('button', { className: 'wsp-btn', onClick: () => setCfgOpen(false) }, ct.close)),
      )) : null
  ...
  function apply(ctx) {
    ...
    slots.inject('settings.section', () => slots.register(
      { name: 'settings.section', id: 'dsh-plugin-market', order: 60, label: () => ({ zh: '插件工坊', en: 'Plugin Workshop', ja: 'プラグイン工房' }[LOCALE]) },
      WorkshopPanel,
    ))
