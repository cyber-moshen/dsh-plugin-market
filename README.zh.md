...
- **搜索**匹配插件名、描述、作者**和标签**；常驻**过滤栏**（安装状态、星标/提交排序、维护状态）；
- **一键安装 / 更新**（后台调用真实 `dsh plugin` CLI，环境坑已全部处理，见下文）；
- 每张卡片右上角的 **GitHub 图标按钮**直达仓库；
- **设置弹窗**：**界面语言切换（中文 / English / 日本語）**、**GitHub Token** 输入（解除 API 限流）、**启动时自动更新**开关；
...
- **界面语言**——中文 / English / 日本語。工坊页面跟随此选择；应用其余部分跟随系统语言。
...
2. 点铅笔（**Edit**）按钮。
3. 复制一条现有条目改成你的插件，插入到 `"plugins": [...]` 对应 `category` 下，并同步更新 `"count"`。
...
```jsonc
{
  "name": "your-plugin-name",                 // 显示名（唯一）
  "owner": "your-github-username",            // 仓库属主
  "url": "https://github.com/you/your-plugin",// github.com 链接
  "category": "tools",                        // ui | theme | session | memory | tools | skill | workflow | notify | model | dev | fun
  "tags": ["工具增强", "自动化"],               // 0-5 个可搜索标签（如 记忆增强 / UI美化）
  "description": { "en": "...", "zh": "..." },// 中英文各一句话
  "npm": null,                                // 有 npm 包就写包名，否则 null
  "install": "dsh plugin --profile web add github:you/your-plugin",
  "added": "2026-01-01"                       // 添加日期 YYYY-MM-DD
}
```

没有合适的分类？在文件顶部的 `categories` 里加一个即可——UI 的过滤器会跟着出现。

## 数据与限流

- **目录**——本仓库 `data/plugins.json`，运行时经镜像链抓取（ghproxy → gh-proxy → ghfast → raw.githubusercontent → github.com/raw）；仓库不可达时用打包快照。
- **星标 / 最近提交**——GitHub API，磁盘缓存 24 小时（`$DSH_HOME/.dsh-plugin-market-cache.json`），后台自动补充。无 Token 时匿名限额**每小时 60 次**；用尽后降级 shields.io 徽章（星标一般能拿到，last-commit 徽章不稳定，所以部分卡片会显示"—"，等配额重置或配置 Token 后由后台循环自动补齐）。
